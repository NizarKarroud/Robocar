...
export default function Toolbar({
  ...
  async function handleFollowWall() {
    if (isFollowingWall) {
      dispatch({ type: "FOLLOW_WALL_STOP" });
      setMode("idle");
      try { await sendFollowWallCommand("stop"); } catch (e) { console.error(e); }
    } else {
      dispatch({ type: "FOLLOW_WALL" });
      setMode("followingWall");
    ...
    <nav className="toolbar panel">
      ...
      <button
        className={`tbtn tbtn--primary ${
          isAvoidTopdown ? "tbtn--active" : "tbtn--outline"
        } ${busy || (isAnyAuto && !isAvoidTopdown) ? "tbtn--disabled" : ""}`}
      ...
      <button
        className={`tbtn tbtn--violet ${
          isBraitenberg ? "tbtn--active" : "tbtn--outline"
        } ${busy || (isAnyAuto && !isBraitenberg) ? "tbtn--disabled" : ""}`}
      ...
      <button
        className={`tbtn ${
          isFollowing ? "tbtn--secondary tbtn--active" : "tbtn--ghost"
        } ${busy || (isAnyAuto && !isFollowing) ? "tbtn--disabled" : ""}`}
      ...
      <button
        className={`tbtn ${
          isFollowingWall ? "tbtn--teal tbtn--active" : "tbtn--ghost"
        } ${busy || (isAnyAuto && !isFollowingWall) ? "tbtn--disabled" : ""}`}
